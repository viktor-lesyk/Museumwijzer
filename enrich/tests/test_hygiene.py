...
def test_scrub_secrets_function():
    raw = {
        "headers": {"Authorization": "Bearer sk-1234567890abcdef12345"},
        "api_key": "tvly-dev-abcdef1234567890",
        "nested": [
            "Contact: sk-abcdef1234567890",
...
def test_no_secrets_in_tracked_git_files():
    ...
    res = subprocess.run(
        ["git", "grep", "-I", "-iE", r"(tvly-[a-zA-Z0-9_-]{10,}|sk-[a-zA-Z0-9_-]{15,}|Bearer\s+[a-zA-Z0-9_.-]{15,})"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
